...
const teamStats = ['Derik','Ana Débora','Derik'].map(buildImplStats);
  ...
  <div class="rank-card">
    <div class="rank-header">
      ...
      <div>
        ...
        <div class="rank-sub">⭐ Zero lojas bloqueadas — pipeline sem travamentos</div>
      </div>
    </div>
      ...
      ${(() => {
        ...
        return `<div class="rank-metric">
          ...
          <div style="font-size:11px;color:#3b82f6;margin-top:8px;padding-top:8px;border-top:1px solid #1e293b">
            ✨ Seu pipeline é <strong>${Math.round((teamStats.find(s=>s.name==='Ana Débora').avgAge - derikStats.avgAge) / teamStats.find(s=>s.name==='Ana Débora').avgAge * 100)}% mais jovem</strong> que o da Ana Débora e <strong>${Math.round((teamStats.find(s=>s.name==='Derik').avgAge - derikStats.avgAge) / teamStats.find(s=>s.name==='Derik').avgAge * 100)}% mais jovem</strong> que o do Derik
          </div>
        </div>`;
      })()}
